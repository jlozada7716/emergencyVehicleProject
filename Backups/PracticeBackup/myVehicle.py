...
class myVehicle:
    ...
    def __init__(self, vehicleDict, id=None, lane=0, speed=0.2, vy=0.05):
        ...
        lead = self.getLeadingVehicle(targetLane=self.lane)
    ...
    def advance(self):
        if self.followTrajectory and len(self.trajectory) > 0:
            ...
            if self.leadingVehicle != None:  # implementing safety conditions and lane change if possible
                ...
                #   self.brakePosition = self.leadingVehicle.x - self.leadingVehicle.length - self.speed * self.tau
                self.brakePosition = (self.leadingVehicle.x - self.x) / (self.speed - self.leadingVehicle.speed)
                # Determine whether the vehicle is within that safePosition
                #   if self.x + self.speed >= self.brakePosition and self.leadingVehicle.y == self.y:
                if self.x + self.speed >= self.brakePosition and self.leadingVehicle.y == self.y:
                    self.matchLeadingSpeed(self.brakePosition)
    ...
    def matchLeadingSpeed(self, safePosition):  #Match the speed of vehicle in front when close enough
        ...
        elif (self.leadingVehicle.x - self.x <= 20):
