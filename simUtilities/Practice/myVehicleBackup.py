...
class myVehicle:
    ...
    def advance(self):
        ...
        if self.emergencyResponse: # Emergency Vehicle has entered the simulation
            ...
            if self.id == -1:
                print("EVehicle's Speed: ", self.speed)
            ...
            if self.leadingVehicle != None and np.mod(self.leadingVehicle.x - self.x, xSimDistance) < 100:  # implementing safety conditions and lane change if possible
                self.updatePrevAttributes()
                if self.leadingVehicle.id == -1 and self.leadingVehicle.x - self.x <= 100:
                    print("STOP")
                if self.leadingVehicle.speed <= 7 and self.leadingVehicle.speed <= self.speed:
                    self.slowSpeedFollowingBehavior()
                else:
                    self.decceleration = 0.35
                    if self.prevSpeed == 0:
                    ...
                    self.acceleration = ((self.alpha * np.power(self.prevSpeed, self.m)) / np.power(
                        np.mod(self.leadingVehicle.prevX - self.prevX, xSimDistance), self.l)) * (
